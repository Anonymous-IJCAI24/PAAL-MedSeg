    ...
    with torch.no_grad():
        for step, sample in enumerate(sample_loader):
            ...
            with autocast(True):
                ...
                uncertainties = -(output*log_probs).sum(1) # -q * log(q)
    ...
    with torch.no_grad():
        for step, sample in enumerate(sample_loader):
            ...
            with autocast(True):
                ...
                output = torch.softmax(output, dim=1).detach().cpu()
                output = output.mean(dim=(2,3)) # NCHW -> NC
                probs = -output.max(1)[0]  # - max(p(y|x))
    ...
    with torch.no_grad():
        for step, sample in enumerate(sample_loader):
            ...
            with autocast(True):
                ...
                output = torch.softmax(output, dim=1).detach().cpu() # NCHW
                output = output.mean(dim=(2,3)) # NCHW -> NC
    ...
    with torch.no_grad():
        for step, sample in enumerate(sample_loader):
            ...
            with autocast(True):
                ...
                output_prob = torch.softmax(output, dim=1).detach().cpu()
                output_prob = output_prob.mean(dim=(2,3)) # NCHW -> NC
                log_probs = torch.log(output_prob)
                uncertainties = -(output_prob*log_probs).sum(1) # -q * log(q)
            output_list.extend(uncertainties)
